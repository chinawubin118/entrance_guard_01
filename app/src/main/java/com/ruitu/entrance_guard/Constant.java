package com.ruitu.entrance_guard;

/**
 * Created by wubin on 2017/5/10.
 */

public interface Constant {
    long TIME_UNIT = 2000;

    String BASE_URL = "http://ceshi.lianxinyi.com.cn:8087/";

    //自己的服务器URL:ceshi.lianxinyi.com.cn/app/
    String URL = "http://ceshi.lianxinyi.com.cn:8087/app/";

    //https://api.seniverse.com/v3/weather/now.json?key=tyc5m2krtykod7bw&location=jinan&language=zh-Hans&unit=c
    //第三方服务器URL(此为心知天气接口)
    String URL_01 = "https://api.seniverse.com/v3/";

//    String DEVICE_MAC = "e0:76:d0:e6:da:0e";//设备mac地址
}
